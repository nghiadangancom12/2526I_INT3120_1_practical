...
fun ReplyListOnlyContent(
    ...
    val emailList = uiState.currentMailboxEmails
    ...
    LazyColumn(
        ...
        items(emailList, key = { it.id }) { email ->
            ReplyEmailListItem(
                email = email,
                selected = false,
                onCardClick = { onEmailSelected(email) }
            )
        }
    }
}
    ...
    Row(
        ...
        LazyColumn(
            ...
            items(emailList, key = { it.id }) { email ->
                ReplyEmailListItem(
                    email = email,
                    selected = uiState.currentSelectedEmail.id == email.id,
                    onCardClick = { onEmailSelected(email) }
                )
            }
        }
        ...
        ReplyDetailsScreen(
            ...
            onBackPressed = { currentActivity.finish() }
        )
    }
}
...
private fun ReplyEmailItemHeader(
    ...
    Row(modifier = modifier) {
        val fullName = stringResource(email.sender.firstName) + " " + stringResource(email.sender.lastName)
        ReplyProfileImage(
